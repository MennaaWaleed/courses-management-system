    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Using the Admin endpoint to retrieve course data regardless of its published/deleted status
                const courseResponse = await getAdminCourseById(id);
                ...
                setLectureCount(course.lectureCount ?? "");
                setPrice(course.price ?? "");
                setCategories(categoriesData || []);
                setCategoryIds(course.categoryIds || []);
        ...
        <div className="edit-course">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                    ...
                    <input type="number" step="0.1" value={courseHours} onChange={(e) => setCourseHours(e.target.value)} required />
                </div>

                <div className="form-group">
                    <label>Lecture Count</label>
                    <input type="number" value={lectureCount} onChange={(e) => setLectureCount(e.target.value)} required />
                </div>
                ...
                <div className="form-group">
                    ...
                    {courseImage && <p>New image: {courseImage.name}</p>}
                </div>
                ...
                <div className="form-group">
                    ...
                    {iconImage && <p>New icon: {iconImage.name}</p>}
                </div>

                <div className="form-group">
                    <label>Course Content PDF</label>
                    {contentUrl && (
                        <p>Current PDF: {contentUrl.split("/").pop()}</p>
                    )}
                    <input type="file" accept=".pdf" onChange={handleContentChange} />
                    {contentFile && <p>New PDF: {contentFile.name}</p>}
                </div>
                ...
                <div className="form-actions">
                    ...
                    <button type="submit" className="save-course-button" disabled={saving}>
            ...
            {showSuccessModal && (
                <div className="success-modal-overlay">
                    <div className="success-modal">
                        ...
                        <button className="success-ok-button" onClick={() => navigate(-1)}>
                            OK
                        </button>
                    </div>
                </div>
