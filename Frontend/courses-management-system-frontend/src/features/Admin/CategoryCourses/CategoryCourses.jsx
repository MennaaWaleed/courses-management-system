...
function CategoryCourses() {
    ...
    const handlePublish = async (course) => {
        try {
            ...
            setCourses((prevCourses) =>
                prevCourses.map((c) => c.id === course.id ? { ...c, published: !c.published } : c)
            );
        } catch (error) {
    ...
    const handleFeature = async (course) => {
        try {
            ...
            setCourses((prevCourses) =>
                prevCourses.map((c) => c.id === course.id ? { ...c, featured: !c.featured } : c)
            );
        } catch (error) {
...
useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const coursesResponse = await getAdminCoursesByCategory(categoryId);
    ...
    if (loading) {
        return <div className="category-courses-loading">Loading...</div>;
    }
        ...
        <div className="category-courses">
            <div className="category-courses-header">
                ...
                <div className="category-title">
                    <h1>
                        <span className="category-name">{category?.categoryName || "Loading..."}</span>{" "}
                        <span className="category-label">Category</span>
                    </h1>
                    <p>{courses.length} {courses.length === 1 ? "Course" : "Courses"}</p>
                </div>
                <button className="create-course-button" onClick={() => navigate(`/admin/categories/${categoryId}/courses/create`)}>
                    + Create New Course
                </button>
            </div>
            ...
            <div className="category-courses-list">
                {courses.length === 0 ? (
                    <div className="no-courses">
                        ...
                        <button onClick={() => navigate(`/admin/categories/${categoryId}/courses/create`)}>
                            Create First Course
                        </button>
                    </div>
                ) : (
                        ...
                        <div className="course-card" key={course.id}>
                            ...
                            <div className="course-info">
                                <div className="course-name-row">
                                    ...
                                    <h2 onClick={() => navigate(`/admin/courses/${course.id}/batches`)} style={{ cursor: "pointer" }} title="View Batches">
                                        {course.courseName}
                                    </h2>
                                </div>
                                <p className="course-description">{course.shortDescription || "No description provided."}</p>
                                ...
                                <div className="course-status-row">
                                    <span className={course.published ? "course-status published" : "course-status not-published"}>
                            ...
                            <div className="course-actions">
                                <button className="edit-course-button" onClick={() => navigate(`/admin/courses/edit/${course.id}`)}>
                                    Edit
                                </button>
                                <button
                                    className={course.featured ? "unfeature-course-button" : "feature-course-button"}
                                    onClick={() => handleFeature(course)}
                                >
                                    {course.featured ? "Unfeature" : "Feature"}
                                </button>
                                <button
                                    className={course.published ? "unpublish-course-button" : "publish-course-button"}
                                    onClick={() => handlePublish(course)}
                                >
                                    {course.published ? "Unpublish" : "Publish"}
                                </button>
                                <button className="delete-course-button" onClick={() => handleDelete(course)}>
                                    Delete
                                </button>
                            </div>
                        </div>
            ...
            {showDeleteModal && (
                <div className="delete-modal-overlay">
                    <div className="delete-modal">
                        ...
                        <h2>Delete Course?</h2>
                        <p>Are you sure you want to delete <strong>{courseToDelete?.courseName}</strong>?</p>
